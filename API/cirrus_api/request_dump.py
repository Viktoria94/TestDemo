...
NB: This code was stolen from requests_toolbelt.util.dump
    I just added the option to dump headers only
"""
import collections
...
def _dump_request_data(request, prefixes, bytearr, proxy_info=None, headers_only=False):
    ...
    if request.body and not headers_only:
...
def _dump_response_data(response, prefixes, bytearr, headers_only=False):
...
def dump_response(response, request_prefix=b'< ', response_prefix=b'> ',
    ...
    _dump_request_data(response.request, prefixes, data,
                       proxy_info=proxy_info, headers_only=headers_only)
    _dump_response_data(response, prefixes, data,
                        headers_only=headers_only)
    return data


def dump_all(response, request_prefix=b'< ', response_prefix=b'> ', headers_only=False):
    ...
    for response in history:
        dump_response(response, request_prefix,
                      response_prefix, data, headers_only=headers_only)
